...
[Bitcoin Core version 0.11.0](https://bitcoincore.org/bin/bitcoin-core-0.11.0/)
...
* P2P network flooded / low-fee transactions
  * -> ballooning of the mempool size
    * POSSIBLE PROBLEMS
      * ⚠️problematic memory use | your node⚠️
        * you can monitor -- via -- RPC command `getmempoolinfo`
        * Solutions: 
          * Solution1: increase the `minrelaytxfee`
            * -> transactions / fewer BTC/kB fee: rejected
              * -> fewer transactions enter | mempool
          * Solution2: restrict `limitfreerelay`
            * if you reduce it -> reduces the speed / mempool can grow -- due to -- free transactions
...
* `limitfreerelay` 
  * := relaying of free transactions
    * by default, 15
    * == number of kB/minute / free transactions will be accepted
...
* release 
  * supports running a FULLY validating node -- WITHOUT -- maintaining a copy of the raw block & undo data | disk

* recap,
  * types of data | blockchain
    * raw blocks as received -- over the -- network (blk???.dat)
    * undo data (rev???.dat)
    * block index 
    * UTXO set (both LevelDB databases)

* databases
  * built -- from the -- raw data

* Block pruning allows Bitcoin Core to delete the raw block and undo data once
it's been validated and used to build the databases

* raw data
  * uses
    * relay blocks -- to -- OTHER nodes
    * handle reorganizations
    * look up old transactions (if -txindex is enabled or via the RPC/REST interfaces)
    * rescan the wallet

* block index
  * continues to hold the metadata
  * ALL blocks | blockchain

The user specifies how much space to allot for block & undo files
* minimum ALLOWED == 550MB
* Note that this is in addition to whatever is required for the block index and UTXO databases
* minimum was chosen so that Bitcoin Core will be able to maintain at least 288 blocks on disk (two days worth of blocks at 10
minutes per block)
* In rare instances it is possible that the amount of space
used will exceed the pruning target in order to keep the required last 288
blocks on disk.

Block pruning works during initial sync in the same way as during steady state,
by deleting block files "as you go" whenever disk space is allocated
* Thus, if
...
*   In the future, nodes with block
...
-reindex
*  Note also that any problem that would cause a user to reindex (e.g.,
...
* big-endian CPU architectures
  ...
  * added | this release
* little-endian
  * specific code -- was replaced with -- endian-neutral constructs

* build system
  * AUTOMATIC detect the target's endianness 

Memory usage optimization
--------------------------

* goal
  * reduce the node's default memory usage
    - accurate UTXO cache size accounting (#6102)
      - `-dbcache` estimate MORE precisely used memory
    - reduce size of per-peer data structure (#6064 and others)
      - -> increases the number of connections / supported with the SAME amount of memory
    - reduce the number of threads (#5964, #5679)
      - == lowers the amount of (virtual) memory needed

Fee estimation changes
----------------------

* improves
  * algorithm / used -- for -- fee estimation
* PREVIOUSLY,
  * if there was insufficient data to give an estimate -> return -1 
* NOW,
  * if there is NO fee or priority high enough / desired confirmation target -> returns -1 
    * ask for an estimate / higher target number of blocks
* NOT uncommon 
  * / 
    * NO fee or 
    * priority < reliably (85%) included | next block
  * `-txconfirmtarget=n` changes from 1 -- to -- 2
...
* release
  * adds functionality / create a NEW circuit -- for -- EVERY peer connection -> software + Tor, is used 
* `-proxyrandomize`
  * by default

* if it's enabled -> every outgoing connection will go -- through a -- DIFFERENT exit node
  * reduces the chance to get unlucky
  * pick 1! exit node / is 
    * malicious
    * widely banned -- from the -- P2P network
  * improves 
    * connection reliability
    * privacy

* if a non-Tor SOCKS5 proxy is configured / 
  * supports authentication
    * TODO: NOT required it this change may cause that proxy to reject connections
* user & password 
  * if they were NOT BEFORE -> is sent 
* setup 
  * EXCEEDINGLY rare, `-proxyrandomize=0`
